...
// Тест 9: Интеграционный тест - открытая дверь вызывает исключение после таймаута
TEST_F(TimedDoorTestUnique, OpenDoorThrowsExceptionAfterTimeout) {
    Timer testTimer;
    testDoor->unlock();
    EXPECT_TRUE(testDoor->isDoorOpened());
    
    // Создаем адаптер и регистрируем в таймере
...
// Тест 10: Интеграционный тест - закрытая дверь не вызывает исключение после таймаута
